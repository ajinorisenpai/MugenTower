...
private:
    ...
    const int VIS_RANGE = 28;
    Texture texture_tile01;
    Texture texture_tile02;
    Texture texture_tile05;
    Texture texture_background;
    Texture texture_shade;
public:
    Stage(String path){
        ...
        texture_tile01 = Texture(Resource(U"Image/map_tile001.png"));
        
        texture_tile05 = Texture(Resource(U"Image/map_tile005.png"));
        texture_tile02 = Texture(Resource(U"Image/map_tile004.png"));
        texture_background = Texture(Resource(U"Image/background.png"));
        texture_shade = Texture(Resource(U"Image/shade.png"));
    ...
    // 0b01000 : ゴール
    // 0b10000 : スタート
    const void sd_draw() const{
        texture_shade.draw(0,0);
    }
        ...
        for(int y = sy-VIS_RANGE;y<sy+VIS_RANGE;y++){
            for(int x =sx-VIS_RANGE;x<sx+VIS_RANGE;x++){
                ...
                if(map_data[x][y] == 0b0001){
                    texture_tile05(0,0,64,64).draw(x*64,y*64);
//                    RoundRect(x*64+2,y*64+2,64-4,64-4,10).draw(Color(80,80,80,150)).drawFrame(3, 3,Color(30, 200, 30));
                   ...
                   texture_tile01(0,0,64,64).draw(x*64,y*64);
...
//                    RoundRect(x*64,y*64,64,64,10).drawFrame(3, 3,Color(200, 40,40));
        ...
        if(tutorial_flag){
            FontAsset(U"Menu")(U"左右キーで移動").drawAt(Vec2(64*4,64*96), Color(255));
            FontAsset(U"Menu")(U"Xキーでジャンプ").drawAt(Vec2(64*16,64*96), Color(255));
        FontAsset(U"Menu")(U"ジャンプ中にXキーでワイヤーアクション").drawAt(Vec2(64*30,64*96), Color(255));
        FontAsset(U"Menu")(U"押す長さによって角度を変えられる").drawAt(Vec2(64*30,64*97), Color(255));
            
        FontAsset(U"Menu")(U"灰色ブロックはワイヤーが引っかからない").drawAt(Vec2(64*50,64*96), Color(255));
       
       FontAsset(U"Menu")(U"Cキーを長押しすれば周りを見ることができる").drawAt(Vec2(64*60,64*97), Color(255)); FontAsset(U"Menu")(U"赤色ブロックに当たるとゲームオーバー").drawAt(Vec2(64*70,64*96), Color(255));
        FontAsset(U"Menu")(U"黄色い扉の前でジャンプするとゲームクリアだ").drawAt(Vec2(64*92,64*96), Color(255));
        }
    ...
    void ExportMapData(){
        ...
        for(auto y : step(100)){
            for(auto x : step(100)){
                map_data[x][y] &= 0b11101111;
            }
        }
        
        int px = (int)StartPos.x/64;
        int py = (int)StartPos.y/64;
        map_data[px][py] = 16;
